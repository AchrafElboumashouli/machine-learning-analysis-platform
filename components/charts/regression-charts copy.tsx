...
export default function RegressionCharts({ visualizationData, activeTab }: RegressionChartsProps) {
  ...
  const errors = residualsData.map((d: any) => Math.abs(d.residual))

  const maxError = errors.length > 0 ? Math.max(...errors) : 1
  const errorBins =
    errors.length > 0
      ? Array.from({ length: 10 }, (_, i) => {
          const min = (i * maxError) / 10
          const max = ((i + 1) * maxError) / 10
            ...
            {actualPredictedData.length > 0 ? (
              <ResponsiveContainer width="100%" height={400}>
                <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
                  ...
                  <XAxis label={{ value: "Actual Values", position: "insideBottomRight", offset: -5 }} />
                  ...
                  <Scatter
                    ...
                    fillOpacity={0.6}
                  />
                  <ReferenceLine stroke="#808080" name="Perfect Fit" />
                </ScatterChart>
              </ResponsiveContainer>
            ) : (
          ...
          <CardContent>
            {residualsData.length > 0 ? (
              <ResponsiveContainer width="100%" height={400}>
                <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
                  <CartesianGrid strokeDasharray="3 3" />
                  <XAxis label={{ value: "Sample Index", position: "insideBottomRight", offset: -5 }} />
                  <YAxis label={{ value: "Residual Value", angle: -90, position: "insideLeft" }} />
                  <Tooltip cursor={{ strokeDasharray: "3 3" }} />
                  <Scatter
                    name="Residuals"
                    data={residualsData}
      ...
      <TabsContent value="error-dist" className="space-y-6 mt-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Frequency distribution of absolute prediction errors</CardDescription>
          </CardHeader>
          <CardContent>
            {errorBins.length > 0 ? (
              <ResponsiveContainer width="100%" height={400}>
